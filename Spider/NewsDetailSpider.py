...
def getnewsdetail(url):
    ...
    newsChannel, sinaNewsId = getnewsID(soup)
    commentApi = 'https://comment.sina.com.cn/page/info?version=1&format=json&channel={}&newsid={}&group=undefined&compress=0&ie=utf-8&oe=utf-8&page=1&page_size=3&t_size=3&h_size=3&thread=1&uid=unlogin_user'.format(
        newsChannel, sinaNewsId)
...
def getnewsID(soup):
    metaData = soup.find(
        'meta', attrs={'name': 'comment'})
    if metaData != None:
        newsIdZip = str(metaData['content'])
    newsIdList = newsIdZip.split(':', 1)
    return newsIdList[0], newsIdList[1]
...
def insertdatabase(news, geturl, Type):
    ...
    commentNum = str(news['commentNum'])
    newsChannel = str(news['newsChannel'])
    sinaNewsId = str(news['sinaNewsId'])

    sql = 'insert into news_api_newsdetail(url, title, date, pic_url, videourl, mainpage, category, readnum, comments, origin, newschannel, sinanewsid) values ("%s", "%s", "%s", "%s", "%s", "%s", %d, 0, "%s", "%s", "%s", "%s")' % (
        url, title, date, pic_url, videourl, mainpage, Type, commentNum, orimainpage, newsChannel, sinaNewsId)
    try:
